...
function main() {
  ...
  const game4Extracted = game4Loader.instantiateGame4Symbols([
    ...
    "clone", "DT", "MAX_STEPS", "SIM_TIME_DILATION",
  ]);
  ...
  const game4ServeResults = results.filter(function(r) { return r.source === "game4.html"; });
  const studioServeResults = results.filter(function(r) { return r.source === "physics-studio.html"; });
  const commonGame4PassCount = game4ServeResults.filter(function(r) { return r.commonServeSuccess; }).length;
  const commonStudioPassCount = studioServeResults.filter(function(r) { return r.commonServeSuccess; }).length;
  const commonServeFailCount = (game4ServeResults.length - commonGame4PassCount) + (studioServeResults.length - commonStudioPassCount);
  const passCount = results.filter(function(r) { return r.pass; }).length;
  const failCount = results.length - passCount;
  const report = formatReport(results, passCount, failCount, crossPassCount, crossResults.length, commonGame4PassCount, commonStudioPassCount);
  fs.writeFileSync(REPORT_FILE, report, "utf8");
  console.log(report);
  process.exit(crossFailCount === 0 && commonServeFailCount === 0 ? 0 : 1);
}
...
function validateWithSolver(source, preset, extracted, simulateServeFn, game4Loader) {
  try {
    ...
    var commonGate = evaluateServeSuccess(simResult);
    var failureReason = checkFailure(simResult, netClearance, firstBounce, secondBounce, expectedFirst, expectedSecond);
    return makeResult(source, preset.id, !failureReason, netClearance, firstBounce, secondBounce, expectedFirst, expectedSecond, simResult.netHit || false, failureReason, commonGate);
  } catch (e) {
...
function buildStudioSandbox() {
  ...
  var symbols = [
    ...
    "makeServeAimCandidate", "getServeLengthProfile",
    "findServeBounceTime", "getServeBounces", "serveBounceScore",
    "cloneVec", "addVec", "clamp", "SIM_TIME_DILATION",
...
function validateWithStudio(source, preset, sandbox) {
  try {
    ...
    var firstBounce = bounces.find(function(b) { return b.z < 0; }) || null;
    var secondBounce = bounces.find(function(b) { return b.z > 0; }) || null;
    var commonGate = evaluateServeSuccess(simResult, sandbox.TABLE);
    var failureReason = checkFailure(simResult, netClearance, firstBounce, secondBounce, expectedFirst, expectedSecond);
    return makeResult(source, preset.id, !failureReason, netClearance, firstBounce, secondBounce, expectedFirst, expectedSecond, simResult.netHit || false, failureReason, commonGate);
  } catch (e) {
    return { source: source, presetId: preset.id, pass: false, failureReason: "simulate_error", error: e.message };
...
function makeResult(source, presetId, pass, netClearance, firstBounce, secondBounce, expectedFirst, expectedSecond, netHit, failureReason, commonGate) {
  return {
    source: source,
    presetId: presetId,
    pass: pass,
    ...
    commonServeSuccess: Boolean(commonGate && commonGate.pass),
    commonServeFailureReasons: commonGate ? commonGate.reasons : ["simulate_error"],
  };
}
...
function formatReport(results, passCount, failCount, crossPassCount, crossTotal, commonGame4PassCount, commonStudioPassCount) {
  var lines = [];
  ...
  var game4Total = results.filter(function(r) { return r.source === "game4.html"; }).length;
  var studioTotal = results.filter(function(r) { return r.source === "physics-studio.html"; }).length;
  lines.push("## Common legal-serve gate");
  lines.push("");
  lines.push("Definition: no net collision; first bounce on server half; second bounce on receiver half; both bounces inside table bounds.");
  lines.push("game4.html: " + commonGame4PassCount + " / " + game4Total + " passed");
  lines.push("physics-studio.html: " + commonStudioPassCount + " / " + studioTotal + " passed");
  lines.push("");
  ...
  lines.push("Exit code: " + (crossFail === 0 && commonGame4PassCount === game4Total && commonStudioPassCount === studioTotal ? 0 : 1) + " (cross-check + common legal-serve gate; individual target-precision diagnostics may still fail)");
