...
左右側旋的名稱、球路彎曲方向與引擎數值符號是三件事，不能再由單一 `sidespin` 正負號互相推導。專案採用的正式資料契約與外部定義集中在 [`docs/SPIN_DIRECTION_CONTRACT.md`](SPIN_DIRECTION_CONTRACT.md)。摘要如下：

| 語意 | 目前 legacy `sidespin` | 球路方向 | 資料欄位 |
|---|---:|---|---|
| 左側旋 | 負值 | 向右彎 | `sideName:"left"`, `curveDirection:"right"` |
| 右側旋 | 正值 | 向左彎 | `sideName:"right"`, `curveDirection:"left"` |

這個正負對應只保證目前引擎的 x-kick compatibility 行為，不代表完整 3D 物理定義。現有核心仍把 `sidespin` 當作沿 `z` 軸的工程代理；真正垂直軸側旋、corkscrew 與 Magnus 飛行力尚未遷移完成，不能在文件中寫成已部署的真實物理模型。
