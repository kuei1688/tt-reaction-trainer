...
>
> 整理基準：2026-07-15
> 本輪狀態：shared-physics-core Phase 1 遷移、批次驗證工具化、MVP 主線規格、左右側旋語意契約與 per-video 發球產生器文件化；未修改核心物理軸向。
>
...
| VAL-003 | 發球 preset / 軌跡檢查 | `physics-studio.html`, `game4.html`, `tools/serve-batch-validation.test.js`, `tools/serve-success-gate.js` | 已工具化 | 2026-07-15 已執行：共用 legal-serve gate 兩份 solver 均 47/47；cross-check 47/47 | 成功定義為無網碰、第一跳在發球方桌內、第二跳在接球方桌內；目標落點誤差與過網高度另列為 solver 診斷，不能解讀成物理真實性 | `physics-studio.html`, `game4.html`, `physics-presets.json` | 檔案存在 | 是 | 仍需 Game 5 實機視覺／手感檢查 |
...
| VAL-012 | 左右側旋語意與 per-video preset contract | `serve-generator.html`, `physics-presets.json`, `videos.json`, `tools/serve-generator-contract.test.js`, `docs/SPIN_DIRECTION_CONTRACT.md` | 已建立 | 2026-07-15 已執行，47 個 approved video 全部通過 | 檢查 `sideName`、`curveDirection`、legacy sign、唯一 `videoId` 與不鏡像路徑；不能取代實機視角驗證 |
...
| CMD-003 | `node tools/serve-batch-validation.test.js` | 對 47 顆 generated preset 跑發球軌跡批次檢查 | 共用 legal-serve gate（兩份 solver）47/47、cross-check 47/47；另列目標精準度診斷 |
| CMD-004 | 待建立 `run-return-batch` | 對 47 顆 generated preset × 技術跑回擊檢查 | 每顆回擊落點、過網高度、成功與否 |
...
| CMD-011 | `node tools/serve-generator-contract.test.js` | VAL-012 左右側旋與 per-video preset contract | 47 個 approved video 的欄位、sign、方向與唯一性檢查 |
...
3. ~~回擊與發球批次已工具化~~：`tools/batch-validation.test.js` 已完成 VAL-004（14 通過 / 0 失敗）；`tools/serve-batch-validation.test.js` 已完成 VAL-003，共用 legal-serve gate 兩份 solver 47/47、cross-check 47/47。仍需補 Game 5 實機視覺／手感與玩家左右輸入驗證。
4. ~~左右側旋語意已建立 contract~~：`tools/serve-generator-contract.test.js` 已完成 VAL-012；仍需把 contract test、solver batch 與人工視角驗證接成固定命令入口。
5. 為 push/chop 建立同時檢查「過網、落點、旋轉方向、弧線高度」的聯合指標。
6. 為 `return-studio.html` 研究機制建立「是否回寫正式遊戲」的審查門檻。
